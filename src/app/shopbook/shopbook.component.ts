...
export class ShopbookComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    //retrieve the cart item from the localstorage 
    let data = localStorage.getItem('cart');
    //if this is not null then convert it to JSON else initialize it as empty
    if (data !== null) {
  ...
  // we will be taking the books response returned from the db call
  // and we will be adding the retrieved   
  handleSuccessfulResponse(response) {
    this.books = new Array<Book>();
    //get books returned by the  spring boot api call
